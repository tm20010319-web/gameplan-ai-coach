...
修复了 `gameplan/ai/advisor.py` 中提前返回导致 DeepSeek 调用代码不可达的问题。现在：

- 配置 API Key 时会调用 DeepSeek
- 未配置 API Key 时返回 `not_configured`
- 外部结果可缓存
- 请求频率限制正常
- 不可信模型输出会失败关闭
- 过期实时响应返回 `stale`
...
修复了空的 `available` 列表被错误替换为全部英雄的问题。现在候选会正确排除：
...
`gameplan/web/picture_analysis.py` 现在会拒绝不在 `KNOWN_HEROES` 中的英雄名称，避免无效或编造的英雄进入打法建议。

### 视觉识别 schema

英雄识别请求会将动态英雄目录写入 `enum`，减少视觉模型输出未知英雄名的情况。

### 任意英雄选择

图片分析页面的“你的英雄”下拉框现在加载完整英雄目录，可以选择任意已收录英雄，不再只显示当前截图中识别出的少数英雄。
...
```text
http://127.0.0.1:8769/monitor?view=screen
```
...
```text
GET  /api/screen/sources
POST /api/screen/frame
```

## 当前使用入口

- 统一工作台：`http://127.0.0.1:8769/monitor`
- 局中观察：`http://127.0.0.1:8769/monitor?view=screen`
- 加载阵容：`http://127.0.0.1:8769/monitor?view=loading`
- 选人助手：`http://127.0.0.1:8769/bp`

## 后续建议

1. 使用真实王者荣耀选人和加载画面验证视觉识别。
2. 验证不同分辨率、多显示器和窗口移动后的采集稳定性。
3. 补齐英雄机制资料的版本、来源、核验日期和审核状态。
4. 继续实现局中事件识别、技能记录和赛后战报。
5. 真实投屏环境下记录 Ollama 推理时间、显存和游戏帧率影响。
